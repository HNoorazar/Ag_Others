...
function addNDBSI_to_IC(IC_){
  var NDBSI_IC = IC_.map(addNDBSI_to_image);
  return NDBSI_IC;
}

function addNDBSI_to_image(image) {
  var v = image.select('SR_B4').subtract(image.select('SR_B3')).rename('v');
  var r = image.expression(
                      '1 - ((SWIR-NIR) / (3 * ee.Number(NIR-RED).abs()) )', {
  ...
  if (k>0){
    var NDBSI = image.normalizedDifference(['SR_B6', 'SR_B2']).rename('NDBSI');
  } else{
    var NDBSI = -ee.Number(image.normalizedDifference(['SR_B6', 'SR_B2']).rename('NDBSI')).abs();
  }
...
function scale_the_damn_bands(image){
  ...
  var SWI1 = image.select('SR_B6').multiply(0.0000275).add(-0.2);
