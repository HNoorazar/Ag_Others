...
def one_sided_smoothing(all_stations_years, window_size=5):
    ...
    for a_loc in locations:
        ...
        for a_year in years:
            ...
            wma_5 = a_signal.rolling(window_size, center=False).apply(lambda a_signal: np.dot(a_signal, weights)/
...
def two_sided_smoothing(all_stations_years, window_size=5):
    ...
    for a_loc in locations:
        ...
        for a_year in years:
            ...
            # weighted moving average. weights are not symmetric here.
            wma = a_signal.rolling(window_size, center=True).apply(lambda a_signal: np.dot(a_signal, weights)/
                                                                       weights.sum(), raw=True)
