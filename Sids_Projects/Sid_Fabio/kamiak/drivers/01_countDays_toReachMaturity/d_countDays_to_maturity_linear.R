...
if (model_type=="observed"){ # 1980-2015.
  start_yr = 1980
  end_yr=2015
} else{ # 2041-2070
  start_yr=2041
...
for(fileName in VIC_grids$file_name){
  ...
  if (file.exists(paste0(data_dir, fileName, ".csv"))){
    ...
    for (a_year in sort(unique(days_to_maturity_tb$year))){
      ...
      # print (head(days_to_maturity_tb, 2))
      # days_to_maturity_tb$cum_solar<-tail(start_to_maturity_tb, 1)$cum_SRAD
      days_to_maturity_tb$cum_solar[days_to_maturity_tb$location==fileName & 
                                          days_to_maturity_tb$year==a_year]=tail(start_to_maturity_tb, 1)$cum_SRAD
      # print (head(start_to_maturity_tb, 2))
      ...
      days_to_maturity_tb$no_days_in_opt_interval[days_to_maturity_tb$location==fileName & 
                                                   days_to_maturity_tb$year==a_year] = dim(optimum_table)[1]

      extreme_cold_tb = start_to_maturity_tb %>%
                        filter(tmin<=veg_params$cold_stress) %>%
                        data.table()

      days_to_maturity_tb$no_of_extreme_cold[days_to_maturity_tb$location==fileName & 
                                             days_to_maturity_tb$year==a_year] = dim(extreme_cold_tb)[1]
      ...
      days_to_maturity_tb$no_of_extreme_heat[days_to_maturity_tb$location==fileName & 
                                              days_to_maturity_tb$year==a_year] = dim(extreme_heat_tb)[1]

    }
  }
}

current_out = paste0(out_dir_base, "/01_countDays_toReachMaturity/",  
                                     veg_type, "_linear", "/") 
