...
dir_base <- paste0(database, "/01_cumGDD_inTimeWindow_Oct18/")
out_dir_base = database # kamiak
...
for (veg_type in veggies){
  ...
  data_dir = paste0(dir_base, veg_type, "_nonlinear_claudio/")
  ...
  medians_data_a_veg_type=all_data_of_veg_type[, .(median_cumGDD_inTW        = median(cumGDD_inTW), 
                                                   median_cumSRAD_inTW       = median(cumSRAD_inTW), 
                                                   median_no_of_extreme_cold = median(no_of_extreme_cold),
                                                   median_no_of_extreme_heat = median(no_of_extreme_heat)),
                                                   by = c("startDoY", "veg_type", "time_period", "state")]



  all_means   = rbind(all_means, means_data_a_veg_type)
  all_medians = rbind(all_medians, medians_data_a_veg_type)
  #####################################################################################
}

current_out = paste0(out_dir_base, "/02_aggregate_cumGDD_inTimeWindow_Oct18/")
if (dir.exists(current_out) == F) {
  dir.create(path = current_out, recursive = T)
}

states=c("CA", "MI", "FL")
all_means$state    <- factor(all_means$state,    levels=states, order=TRUE)
all_means$startDoY <- factor(all_means$startDoY, levels=sort(unique(all_means$startDoY)), order=TRUE)
all_means$veg_type <- factor(all_means$veg_type, levels=sort(unique(all_means$veg_type)), order=TRUE)
...
setorder(all_means, cols = "state", "veg_type", "startDoY")
setorder(all_medians, cols = "state", "veg_type", "startDoY")

write.csv(all_means, 
          file = paste0(current_out, "cumGDDMeans_withinState_inTW_allVegs_allstartDoYs.csv"), 
          row.names=FALSE)

write.csv(all_medians, 
          file = paste0(current_out, "cumGDDMedians_withinState_inTW_allVegs_allstartDoYs.csv"), 
          row.names=FALSE)


CA_means = all_means[all_means$state=="CA", ]
CA_medians = all_medians[all_medians$state=="CA", ]

CA_means$state    <- factor(CA_means$state,    levels=states, order=TRUE)
CA_means$startDoY <- factor(CA_means$startDoY, levels=sort(unique(CA_means$startDoY)), order=TRUE)
CA_means$veg_type <- factor(CA_means$veg_type, levels=sort(unique(CA_means$veg_type)), order=TRUE)
...
setorder(CA_means, cols = "state", "veg_type", "startDoY")
setorder(CA_medians, cols = "state", "veg_type", "startDoY")

write.csv(CA_means, 
          file = paste0(current_out, "CA_cumGDDMeans_withinState_inTW_allVegs_allstartDoYs.csv"), 
          row.names=FALSE)

write.csv(CA_medians, 
          file = paste0(current_out, "CA_cumGDDMedians_withinState_inTW_allVegs_allstartDoYs.csv"), 
          row.names=FALSE)
