...
for (a_subdir in sub_dirs){
  ...
  mean_days_to_maturity_plot<- annual_TS_3(d1=annual_means_within_CRD, 
  ...
  for (a_crd in chosen_CRD_alph){
    ...
    mean_days_to_maturity_plot<- annual_TS_3(d1=annual_means_within_CRD_subset, 
                                             y_colname=col_to_plot, fil="time_period",
                                             y_label="average cumulative SR over growth period")
